...
Tests are colocated as `src/**/*.test.ts` (run by vitest) and excluded from the build by `tsconfig.json`. One of them, `src/server/stdio.test.ts`, spawns `src/server/stdio-fixture.mjs` as a child process and drives it over a real pipe — it imports the BUILT `dist/`, because the bug it pins (`server/discover` → `-32601`) is invisible to an in-memory transport. It runs `tsc -b` in `beforeAll` (incremental, a sub-second no-op when `dist/` is current) so it can never pass against a stale build. The fixture is `.mjs` on purpose: `tsconfig.json#include` is `src/**/*.ts`, so it is never compiled and never published. There is **no lint step** and **no enforced coverage threshold** (`@vitest/coverage-v8` is installed but no thresholds are configured) — `npm test` is the bar, and it typechecks before it runs a single test. CI runs on Node **26**.
...
- **server** — `createMcpServer` / `runMcp` / `withGracefulShutdown`. Transport- and domain-agnostic; the caller builds its client in `deps` so the deferred-config-error pattern (server boots before creds exist) is preserved. **`runMcp` serves through the SDK's `serveStdio` entry, with `createMcpServer` as the factory, and returns a `StdioServerHandle` synchronously.** It must not go back to `server.connect(new StdioServerTransport())`: under SDK v2 the protocol era is instance state and only a serving entry marks an instance modern, so a hand-wired connect leaves every server 2025-era and answers `server/discover` with `-32601 Method not found` — which is what shipped to every fleet consumer at once (60 repos call `runMcp`) (found by @bschrib, chrischall/skylight-mcp#182). Consequences the docblocks spell out and callers depend on: the tool registrars run **per served instance**, so `deps` is the only safe home for anything expensive or credential-bearing; the banner is printed by `runMcp` at boot rather than inside the factory, so one boot is one banner; and `legacy` defaults to `'serve'`, because `'reject'` silently drops every pre-2026 host.
...
- **Error messages must never echo a credential.** `formatApiError` and `errorResult` run every upstream body through `redactSecrets` (Bearer/Basic headers, Cookie/Set-Cookie values, JWTs, `sk-`/`ghp_`/`xox?-`/`AIza`/`AKIA`/`whsec_` key shapes, secret URL query params incl. session ids and `x-…` credential names, secret JSON values quoted or numeric, `"cookie"`/`"set-cookie"` JSON values) *before* truncating. A 401 yields a fixed string, never the token. When touching anything that formats an upstream response into a thrown/returned message, keep redaction first.
- **Every redaction pattern must stay linear.** `redactSecrets` runs on the WHOLE upstream body before the 500-char cut, so a superlinear regex is a one-response DoS (the 2026-09-24 audit measured 40 s on 200 KB from an unbounded `\b`-anchored JWT scan and a `{0,}` header-name quantifier). Anchor scans on a literal or on the START of a charset run (negative lookbehind over the run's charset, never `\b` when `-` is in the charset), bound every name quantifier (`{0,40}`), and add the adversarial input to `src/errors/redos.test.ts` — it times the whole chain against 200 KB runs with a 100 ms budget. `truncateErrorMessage` also caps its input at `ERROR_REDACTION_INPUT_MAX` (64 KB) before redacting; keep that cap ahead of the redactor.
